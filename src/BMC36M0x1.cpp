...
Description:    UART communication with the BM36C521x-0   
History:      
V1.0.0   -- initial version；2022-03-09；Arduino IDE : v1.8.16
***********************************************************/
#include  "BMC36M0x1.h"
...
Parameters:  S_Addr: short address
             datalength: data length(=<60)
             data[]: The starting store to send data
Return:  
Others:           
**********************************************************/
uint8_t BMC36M0x1::writePairPackage(uint32_t shortAddr,uint8_t len,uint8_t data[])
{
...
Parameters:  len: length of the deviInfo[] 
             deviInfo[]:Information that needs to be written(=<31bytes) 
...
Parameters:  deviInfo[]: Used to store the obtained information (=<31bytes)
